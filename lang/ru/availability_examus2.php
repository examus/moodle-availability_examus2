...
//Redirect screen
$string['redirect_btn'] = 'Перейти в Examus';
$string['redirect_title'] = 'Перенаправляем в Examus';
...
$string['privacy:metadata:availability_examus2_external'] = 'Для интеграции с системой прокторинга Экзамус данные пользователя передаются через API, это обеспечивает аутентификацию пользователя, создание сессии прокторинга и участвует в проверке личности. Экспортируемые поля необходимы для создания и отслеживания сессий прокторинга.';
$string['privacy:metadata:availability_examus2_external:username'] = 'Логин передается для однозначной идентификации аккаунта пользователя в системе Экзамус, что позволяет корректно связать сессию тестирования с профилем. Используется как уникальный идентификатор для сессии.';
$string['privacy:metadata:availability_examus2_external:firstname'] = 'Имя передается для точной идентификации в отчетах и интерфейсе прокторинга, обеспечивая соответствие данным учебного заведения, является опциональным и не обязательным полем.';
$string['privacy:metadata:availability_examus2_external:lastname'] = 'Фамилия используется для сопоставления личности с официальными данными, что упрощает административный контроль, является опциональным и не обязательным полем.';
$string['privacy:metadata:availability_examus2_external:middlename'] = 'Отчество (если предусмотрено) включается для соблюдения стандартов именования и полноты отображения ФИО, является опциональным и не обязательным полем.';
$string['privacy:metadata:availability_examus2_external:email'] = 'Электронная почта необходима для отправки уведомлений, идентификации пользователя в системе Экзамус.';
$string['privacy:metadata:availability_examus2_external:userpicture'] = 'Фото профиля используется для визуальной проверки личности во время сессии. В качестве фотографии используется аватар из профиляв Moodle. Предотвращает подмену сдающего и обеспечивает соответствие требованиям. Является опциональным и не обязательным полем.';

//Hint text

$string['hint'] = 'Справка по использованию';
